...
function renderSchedule(){
 const startHour=7,endHour=22,hourHeight=64,total=(endHour-startHour)*hourHeight;
 const today=new Date(),monday=new Date(today);monday.setDate(today.getDate()-((today.getDay()+6)%7));
 let head='<div class="week-spacer"></div>';
 for(let i=0;i<7;i++){const d=new Date(monday);d.setDate(monday.getDate()+i);const jsDay=(i+1)%7;head+=`<div class="ios-day ${jsDay===today.getDay()?'today':''}"><span>${['一','二','三','四','五','六','日'][i]}</span><strong>${d.getDate()}</strong></div>`}
 $('#iosWeekHeader').innerHTML=head;
 let grid='<div class="time-axis">';for(let h=startHour;h<=endHour;h++)grid+=`<span style="top:${(h-startHour)*hourHeight}px">${String(h).padStart(2,'0')}:00</span>`;grid+='</div>';
 for(let i=0;i<7;i++){const day=(i+1)%7;grid+=`<div class="day-lane" data-calendar-day="${day}" style="height:${total}px;background-size:100% ${hourHeight}px">`;
  courses.filter(c=>+c.day===day).forEach(c=>{const top=Math.max(0,(minutes(c.start)-startHour*60)/60*hourHeight),height=Math.max(32,(minutes(c.end)-minutes(c.start))/60*hourHeight);grid+=`<div class="ios-event" style="--accent:${colors[c.color]};top:${top}px;height:${height}px" data-event="${c.id}"><button class="remove" data-delete-course="${c.id}" aria-label="删除课程">×</button><strong>${esc(c.name)}</strong><span>${c.start}–${c.end}</span><small>${esc(c.location||'')}</small></div>`});
  grid+='</div>';
 }
 $('#iosTimeGrid').innerHTML=grid;
 const wrap=$('.calendar-shell');if(wrap&&!wrap.dataset.positioned){wrap.dataset.positioned='1';setTimeout(()=>{wrap.scrollTop=Math.max(0,(new Date().getHours()-startHour-1)*hourHeight);wrap.scrollLeft=Math.max(0,((today.getDay()+6)%7-1)*92)},0)}
}
...
$('#iosTimeGrid').addEventListener('click',e=>{const lane=e.target.closest('.day-lane');if(!lane||e.target.closest('.ios-event'))return;const rect=lane.getBoundingClientRect(),hour=7+(e.clientY-rect.top)/64;let mins=Math.round((hour*60)/30)*30;mins=Math.max(7*60,Math.min(21*60,mins));const fmt=m=>`${String(Math.floor(m/60)).padStart(2,'0')}:${String(m%60).padStart(2,'0')}`;const form=$('#courseForm');form.elements.day.value=lane.dataset.calendarDay;form.elements.start.value=fmt(mins);form.elements.end.value=fmt(Math.min(22*60,mins+90));$('#courseDialog').showModal()});
